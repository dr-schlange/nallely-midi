...
export const VDevicePlaceholder = memo(
...
const SortableVDevice = memo(
	({ device, onClick, selectedSections, onDrag }: SortableVDeviceProps) => {
		const deviceUid = useMemo(() => devUID(device), [device]);
		const isSelected = useMemo(
			() => selectedSections.some((s) => s.startsWith(deviceUid)),
			[selectedSections, deviceUid],
		);
		...
		return (
			<div ref={setNodeRef} style={style} {...attributes} {...listeners}>
				<VDevice device={device} onClick={onClick} selected={isSelected} />
			</div>
		);
	},
);
