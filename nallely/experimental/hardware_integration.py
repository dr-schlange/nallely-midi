...
class LISA(VirtualDevice):
    ...
    * shift_cv [0, 1] init=0 <rising, falling>: access to second layer of buttons/sliders
    ...
    def _setup_lfos(self, lisa: Lisa, minilab: Minilab3):
        ...
        lisa.general.master_volume = minilab.buttons.b1.scale(0, 127, method="log")
        lisa.general.gain = minilab.buttons.b2.scale(0, 127, method="log")
    ...
    def _setup_adsr(self, lisa: Lisa, minilab: Minilab3):
        ...
        # map non-shift needs to be declared first
        self.adsr_filter.attack_cv = minilab.sliders.s1
        self.adsr_filter.decay_cv = minilab.sliders.s2
        self.adsr_filter.sustain_cv = minilab.sliders.s3
        self.adsr_filter.release_cv = minilab.sliders.s4
        self.adsr_filter.set_pause_cv = minilab.pads.p5

    def collect_mappings(self, minilab: Minilab3):
        non_shift_map = []
        shift_map = []
        btns = minilab.buttons
        sliders = minilab.sliders
        for i in range(1, 9):
